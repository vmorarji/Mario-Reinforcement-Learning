...
class FireResetEnv(gym.Wrapper):
    def __init__(self, env, fire_max=10):
        """Take action on reset for environments that are fixed until firing."""
        gym.Wrapper.__init__(self, env)
        self.fire_max = fire_max
        assert env.get_action_meanings()[1] == 'right'
        assert len(env.get_action_meanings()) >= 3

    def reset(self, **kwargs):
        self.env.reset(**kwargs)
        fire_len = self.unwrapped.np_random.randint(1, self.fire_max)
        fire = self.unwrapped.np_random.randint(1, 3, fire_len) 
        
        for i in range(len(fire)):
            obs, _, done, _ = self.env.step(fire[i])
            if done:
                obs = self.env.reset(**kwargs)        
        return obs
